...
Vertical Slice Architecture sẽ phù hợp hơn với hệ thống sử dụng kết hợp Microservices & CQRS. Nơi mà mỗi Serivice hay các thành phần của nó là Command và Query có thể hoạt động độc lập mà không cần biết đến các Service hay Command và Query khác. Số lượng API cho mỗi serivce cũng không quá lớn như monolithic.
