...
Mediator hay `Người điều phối` là một `behavioral design pattern` giúp bạn giảm được sự phụ thuộc lộn xộn giữa các object. Mediator pattern sẽ hạn chế các object giao tiếp trực tiếp với nhau. Thay vào đó, chúng sẽ tương tác thông qua một đối tượng trung gian (Mediator).
